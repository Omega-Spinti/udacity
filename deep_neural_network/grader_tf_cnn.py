...
def get_result(input_array, student_func):
    ...
    with tf.Session() as sess:
        ...
        if did_pass:
            ...
            result['values'] = ['your output shape: %s' % their_shape]
        else:
            result['values'] = ['correct shape: %s' % our_shape]
            result['output'] = ['your output shape: %s' % their_shape]
...
def run_grader(input_array, student_func):
    ...
    if grader_result['is_correct']:
        ...
        comment = "Not quite. The correct output shape is {gt_shape} while your output shape is {student_func_shape}."
    else:
        test_error = grader_result['error']
        comment = "Something went wrong with your submission: %s" % test_error
